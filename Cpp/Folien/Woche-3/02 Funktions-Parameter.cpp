...
void print_predictions(
    ...
    double (*prediction_algorithm)(const std::vector<double>&))
{
    std::cout << "Prediction: " << prediction_algorithm(prices) << "\n";
}
...
// %%
double average_prediction_2(const std::vector<double>& prices)
{
    return std::accumulate(prices.cbegin(), prices.cend(), 0.0) / prices.size();
}

// %%
print_predictions(prices, average_prediction_2);
