...
// %%
std::string value_string(int* ptr) { return ptr ? std::to_string(*ptr) : "nullptr"s; }

// %% tags=["subslide"] slideshow={"slide_type": "subslide"}
class IntOnHeap {
public:
    IntOnHeap(int value) : value_{new int{value}} {
        std::cout << "Constructing IntOnHeap(" << value_string(value_) << ")\n";
    }

    ~IntOnHeap() {
        std::cout << "Destructing IntOnHeap(" << value_string(value_) << ")\n";
        delete value_;
    }

    IntOnHeap(const IntOnHeap&) = delete;
    IntOnHeap& operator=(const IntOnHeap&) = delete;

    IntOnHeap(IntOnHeap&& other) : value_{other.value_} {
        std::cout << "Move-Constructing IntOnHeap(" << value_string(value_) << ")\n";
        other.value_ = nullptr;
    }

    IntOnHeap& operator=(IntOnHeap&& other) {
        std::cout << "Move-Assigning IntOnHeap(" << value_string(value_) << ")\n";
        delete value_;
        std::cout << "Other value = " << *other.value_ << "\n";
        value_ = other.value_;
        other.value_ = nullptr;
        return *this;
    }
    ...
    int get() const {
        if (!value_) {
            throw std::logic_error{"Trying to get value of destroyed object."};
        }
        return *value_;
    }
